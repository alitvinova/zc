...
extern CFeeRate minRelayTxFee; //= CFeeRate(/*DEFAULT_MIN_RELAY_TX_FEE*/0.01);
extern CCoinsViewCache *pcoinsTip; // = NULL;
extern ZCJoinSplit *pzcashParams; // = ZCJoinSplit::Unopened();
extern BlockMap mapBlockIndex;

extern int find_output(UniValue obj, int n);
...
bool CSender::create_tx() {
    ...
    std::string s = strprintf("z_sendmany finished");
    if (success) {
 ...
 */
void CSender::sign_send_raw_transaction(UniValue obj)
{   
...
bool CSender::find_utxos(bool fAcceptCoinbase=false) {
...
bool CSender::find_unspent_notes() {
...
UniValue CSender::perform_joinsplit(AsyncJoinSplitInfo & info) {
    std::vector<boost::optional < ZCIncrementalWitness>> witnesses;
    uint256 anchor;
    {
        //LOCK2(cs_main, pwalletMain->cs_wallet);
...
UniValue CSender::perform_joinsplit(AsyncJoinSplitInfo & info, std::vector<JSOutPoint> & outPoints) {
    std::vector<boost::optional < ZCIncrementalWitness>> witnesses;
    uint256 anchor;
    {
        //LOCK(cs_main);
...
UniValue CSender::perform_joinsplit(
        AsyncJoinSplitInfo & info,
        std::vector<boost::optional < ZCIncrementalWitness>> witnesses,
        uint256 anchor)
{
...
void CSender::add_taddr_outputs_to_tx() {
...
void CSender::add_taddr_change_output_to_tx(CAmount amount) {
...
boost::array<unsigned char, ZC_MEMO_SIZE> CSender::get_memo_from_hex_string(std::string s) {
